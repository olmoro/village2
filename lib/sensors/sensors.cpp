...
void boilerStateChange(rLoadController *ctrl, bool state, time_t duration)
{
  if (thermostatNotify) {
    if (state) {
      tgSend(CONTROL_THERMOSTAT_NOTIFY_KIND, CONTROL_THERMOSTAT_NOTIFY_PRIORITY, 
        CONTROL_THERMOSTAT_NOTIFY_ALARM, CONFIG_TELEGRAM_DEVICE, 
...
bool boilerMqttPublish(rLoadController *ctrl, char* topic, char* payload, bool free_topic, 
                         bool free_payload)
{
  return mqttPublish(topic, payload, CONTROL_THERMOSTAT_QOS, CONTROL_THERMOSTAT_RETAINED, 
    free_topic, free_payload);
}
...
bool sensorsBoilerTempCheck()
{
  ...
  // Получаем текущую температуру с сенсора (она хранится во втором "хранилище") 
  // без физического чтения данных с шины
  float tempIndoor = NAN;
  if (sensorIndoor.getStatus() == SENSOR_STATUS_OK) {
    tempIndoor = sensorIndoor.getValue(false).filteredValue;
  };

  // Если удалось считать температуру, проверяем её в зависимости от текущего состояния нагрузки
  if (!isnan(tempIndoor)) {
    if (oldState) {
      // Сейчас котел включен. Выключить мы его должны, когда температура достигнет 
      // порогового + 1/2 гистерезиса
      return tempIndoor < (thermostatInternalTemp + 0.5 * thermostatInternalHyst);
    } else {
      // Сейчас котел выключен. Включить мы его должны, когда температура снизится 
      // до порогового - 1/2 гистерезиса
      return tempIndoor < (thermostatInternalTemp - 0.5 * thermostatInternalHyst);
    };
  };
  return false;
...
static bool sensorsPublish(rSensor *sensor, char* topic, char* payload, 
                             const bool free_topic, const bool free_payload)
{
  return mqttPublish(topic, payload, CONFIG_MQTT_SENSORS_QOS, CONFIG_MQTT_SENSORS_RETAINED, 
                       free_topic, free_payload);
}
...
static void sensorsInitSensors()
{
  ...
  sensorOutdoor.initExtItems(SENSOR_OUTDOOR_NAME, SENSOR_OUTDOOR_TOPIC, false,
    (gpio_num_t)CONFIG_GPIO_TEMP_OUTDOOR, ONEWIRE_NONE, 1, DS18x20_RESOLUTION_12_BIT, true, 
    &siOutdoorTemp,
  ...
  static rTemperatureItem siIndoorTemp(nullptr, CONFIG_SENSOR_TEMP_NAME, CONFIG_FORMAT_TEMP_UNIT,
    ...
    (gpio_num_t)CONFIG_GPIO_TEMP_INDOOR, ONEWIRE_NONE, 1, DS18x20_RESOLUTION_12_BIT, true, 
    &siIndoorTemp,
  ...
  sensorBoiler.initExtItems(SENSOR_BOILER_NAME, SENSOR_BOILER_TOPIC, false,
    (gpio_num_t)CONFIG_GPIO_TEMP_BOILER, ONEWIRE_NONE, 1, DS18x20_RESOLUTION_12_BIT, true, 
    &siBoilerTemp,
      ...
      if ((cmd != nullptr) && (strcasecmp(cmd, CONFIG_SENSOR_COMMAND_EXTR_RESET) == 0)) {
        ...
        if ((sensor == nullptr) || (strcasecmp(sensor, CONFIG_SENSOR_COMMAND_SENSORS_PREFIX) == 0)) {
          ...
          if (strcasecmp(sensor, SENSOR_OUTDOOR_TOPIC) == 0) {
            ...
            #if CONFIG_TELEGRAM_ENABLE
              tgSend(CONFIG_SENSOR_COMMAND_KIND, CONFIG_SENSOR_COMMAND_PRIORITY, 
                CONFIG_SENSOR_COMMAND_NOTIFY, CONFIG_TELEGRAM_DEVICE,
...
// --------------------------------------------------------------------------------------
  ...
  while (1) {
    ...
    if (sensorOutdoor.getStatus() == SENSOR_STATUS_OK) {
      rlog_i("OUTDOOR", "Values raw: %.2f °С | out: %.2f °С | min: %.2f °С | max: %.2f °С", 
        sensorOutdoor.getValue(false).rawValue,  
    ...
    if (sensorIndoor.getStatus() == SENSOR_STATUS_OK) {
      rlog_i("INDOOR", "Values raw: %.2f °С | out: %.2f °С | min: %.2f °С | max: %.2f °С", 
        sensorIndoor.getValue(false).rawValue, 
        sensorIndoor.getValue(false).filteredValue,
        sensorIndoor.getExtremumsDaily(false).minValue.filteredValue, 
        sensorIndoor.getExtremumsDaily(false).maxValue.filteredValue);
    };
    ...
    if (sensorIndoor.getStatus() == SENSOR_STATUS_OK) {
      tempMonitorIndoor.checkValue(sensorIndoor.getValue(false).filteredValue);
    };
    if (sensorBoiler.getStatus() == SENSOR_STATUS_OK) {
    ...
    if (_sensorsNeedStore) { _sensorsNeedStore = false; sensorsStoreData(); };
        ...
        if (sensorOutdoor.getStatus() == SENSOR_STATUS_OK) {
          omValues = concat_strings_div(omValues, 
            malloc_stringf("p1=%.3f",   //&p2=%.2f", 
              sensorOutdoor.getValue(false).filteredValue, sensorOutdoor.getValue(false).filteredValue),
            "&");
        };
        // Комната
        if (sensorIndoor.getStatus() == SENSOR_STATUS_OK) {
          omValues = concat_strings_div(omValues, 
            malloc_stringf("p2=%.3f",   //&p4=%.2f", 
              sensorIndoor.getValue(false).filteredValue, sensorIndoor.getValue(false).filteredValue),
            "&");
        };
        // Котёл
        if (sensorBoiler.getStatus() == SENSOR_STATUS_OK) {
          omValues = concat_strings_div(omValues, 
            malloc_stringf("p3=%.3f", 
              sensorBoiler.getValue(false).filteredValue),
            "&");
        };
        // Отправляем данные
    ...
    // thingspeak.com
      ...
      if (statesInetIsAvailabled() && timerTimeout(&tsSendTimer)) {
        ...
        if (sensorIndoor.getStatus() == SENSOR_STATUS_OK) {
          tsValues = concat_strings_div(tsValues, 
            malloc_stringf("field2=%.3f",   //&field4=%.2f", 
              sensorIndoor.getValue(false).filteredValue, sensorIndoor.getValue(false).filteredValue),
            "&");
        };
        // Котёл
...
bool sensorsTaskStart()
{
    ...
    _sensorsTask = xTaskCreateStaticPinnedToCore(sensorsTaskExec, sensorsTaskName, 
      CONFIG_SENSORS_TASK_STACK_SIZE, NULL, CONFIG_TASK_PRIORITY_SENSORS, sensorsTaskStack, 
       &sensorsTaskBuffer, CONFIG_TASK_CORE_SENSORS);
  #else
    xTaskCreatePinnedToCore(sensorsTaskExec, sensorsTaskName, 
      CONFIG_SENSORS_TASK_STACK_SIZE, NULL, CONFIG_TASK_PRIORITY_SENSORS, 
        &_sensorsTask, CONFIG_TASK_CORE_SENSORS);
