...
#if CONFIG_RX_STATIC_ALLOCATION   // не получилось
//StaticQueue_t _alarmQueueBuffer;
StaticTask_t _rxTaskBuffer;
StackType_t _rxTaskStack[CONFIG_RX_TASK_STACK_SIZE*2];
//uint8_t _alarmQueueStorage[CONFIG_RX_TASK_QUEUE_SIZE * RX_TASK_QUEUE_ITEM_SIZE];
xTaskCreateStaticPinnedToCore(rxIsrHandler, "Pult_task", 
    CONFIG_RX_TASK_STACK_SIZE*2, 
    NULL, 
    CONFIG_TASK_PRIORITY_IR_RX, 
    _rxTaskStack, 
    &_rxTaskBuffer, 
    CONFIG_TASK_CORE_IR_RX); 
#else
