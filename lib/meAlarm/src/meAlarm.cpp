...
{ 
  if (_alarmExitLock) {
    _alarmExitLock = false;
      ...
      tgSend(MK_SECURITY, CONFIG_ALARM_NOTIFY_PRIORITY_MODE_CHANGE, CONFIG_NOTIFY_TELEGRAM_ALARM_ALERT_MODE_CHANGE, 
        CONFIG_TELEGRAM_DEVICE, CONFIG_NOTIFY_TELEGRAM_ALARM_MODE_ACTIVATED);
...
static void alarmTimeEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
  if (event_id == RE_TIME_SILENT_MODE_ON) {
    if (_ledExec) ledTaskSend(_ledExec, lmEnable, 1, 0, 0);
    if (_ledAlarm) ledTaskSend(_ledAlarm, lmEnable, 1, 0, 0);
  } else if (event_id == RE_TIME_SILENT_MODE_OFF) {
    if (_ledExec) ledTaskSend(_ledExec, lmEnable, 0, 0, 0);
...
bool alarmEventCheckAddress(input_data_t* data, alarmSensorHandle_t sensor)
{
  switch (sensor->type) {
    ...
    case AST_RXIR_16A_16C:
      return (data->source == IDS_RXIR) && ((data->rxIR.value >> 16) == sensor->address);
    case AST_WIRED:
...
static bool alarmEventCheckValueSet(input_data_t* data, alarm_sensor_type_t type, alarmEventHandle_t event)
{
  switch (type) {
    case AST_RXIR_GENERIC:
      return true;
    case AST_RXIR_16A_16C:
      return (data->rxIR.value & 0x00FF) == event->value_set; // (data->rxIR.value & 0x0f) == event->value_set;
    case AST_MQTT:
...
static bool alarmEventCheckValueClr(input_data_t* data, alarm_sensor_type_t type, alarmEventHandle_t event)
{
  switch (type) {
    case AST_RXIR_GENERIC:
      return false;
    case AST_RXIR_16A_16C:
      return (data->rxIR.value & 0x00FF) == event->value_clr; //(data->rxIR.value & 0x0f) == event->value_clr;
    case AST_MQTT:
...
{
  // Заменить обработкой пакета NEC
  if (data->source == IDS_GPIO) {
    ...
    rlog_i(logTAG, "Incoming message:: end of packet: %d, source: RXIR, value: 0x%.8X, address: 0x%.8X, command: 0x%02X, count: %d", 
      end_of_packet, data->rxIR.value, data->rxIR.value >> 16, data->rxIR.value & 0x00FF, data->count);
...
static void alarmTaskExec(void *pvParameters)
{
  ...
  while (1) 
  {
    ...
    {
      // Send signal to LED
      if ((data.source == IDS_RXIR) && (_ledExec)) 
      {
        ledTaskSend(_ledExec,                         // Заданным светодиодом
                     lmFlash,                         // мигнуть
                     CONFIG_ALARM_INCOMING_QUANTITY,  // одной вспышкой
                     CONFIG_ALARM_INCOMING_DURATION,  // длительностью 50мс
                     CONFIG_ALARM_INCOMING_INTERVAL); // с паузой 50мс
      };
      ...
      {
        // Обработка сигналов от ИК датчика
        if ((data.source == bufIR.source) && (data.rxIR.value == bufIR.rxIR.value)) 
        {
          // Если это не первый сигнал в пакете...
          bufIR.count++;
          ...
          {
            // Если количество сигналов превысило пороговое значение, отправить их на обработку
            // rlog_d(logTAG, "Process RXIR signal (threshold): protocol=%d, value=0x%.8X, count=%d", bufIR.rxIR.value, bufIR.rxIR.value, bufIR.count);

            rxIR_processed = alarmProcessIncomingData(&bufIR, false); // Заменить обработкой пакета NEC
          };
        } 
        else 
        {
          ...
          {
            // Если предыдущий сигнал не был обработан, отправить его на обработку
            // rlog_d(logTAG, "Process RXIR signal (changed): protocol=%d, value=0x%.8X, count=%d", bufIR.rxIR.value, bufIR.rxIR.value, bufIR.count);
            alarmProcessIncomingData(&bufIR, true); // Заменить обработкой пакета NEC
          };
          // Set new data to last and reset the counter (we don't really need it), instead we will count the number of packets
          /* Установите для новых данных значение last и сбросьте счетчик (на самом деле
              он нам не нужен), вместо этого мы будем подсчитывать количество пакетов*/
          ...
          {
            /* Если пороговое значение не достигнуто, немедленно обработайте сигнал */
            // rlog_d(logTAG, "Process RXIR signal (threshold): protocol=%d, value=0x%.8X, count=%d", bufIR.rxIR.value, bufIR.rxIR.value, bufIR.count);
...
bool alarmTaskCreate(ledQueue_t siren, ledQueue_t flasher, ledQueue_t ledExec, ledQueue_t ledAlarm, ledQueue_t ledRXIR, cb_alarm_change_mode_t cb_mode)
{
  if (!_alarmTask) {
    _siren = siren;
    _flasher = flasher;
