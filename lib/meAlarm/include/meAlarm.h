...
static const uint16_t ASR_EXEC         = BIT7;   // ( ASR_BUZZER ) Зелёный светодиод
...
static const uint16_t ASR_RELAY2_ON    = BIT11;  // Включить реле2 (нагрузку)
static const uint16_t ASR_RELAY2_OFF   = BIT12;  // Выключить реле2 (нагрузку)
...
static const uint16_t ASRS_ALARM_NOTIFY = ASR_ALARM_INC | ASR_MQTT_EVENT | ASR_MQTT_STATUS | ASR_TELEGRAM | ASR_EXEC;
static const uint16_t ASRS_ALARM_SILENT = ASR_ALARM_INC | ASR_MQTT_EVENT | ASR_MQTT_STATUS | ASR_TELEGRAM | ASR_EXEC | ASR_FLASHER;
static const uint16_t ASRS_ALARM_SIREN  = ASR_ALARM_INC | ASR_MQTT_EVENT | ASR_MQTT_STATUS | ASR_TELEGRAM | ASR_EXEC | ASR_SIREN | ASR_FLASHER;
static const uint16_t ASRS_POWER_ON     = ASR_MQTT_EVENT | ASR_MQTT_STATUS | ASR_TELEGRAM | ASR_FLASHER;
static const uint16_t ASRS_POWER_OFF    = ASR_ALARM_INC | ASR_MQTT_EVENT | ASR_MQTT_STATUS | ASR_TELEGRAM | ASR_EXEC | ASR_FLASHER;
 ...
 * @param ledExec Ссылка-указатель на светодиод, индицирующий режим работы (бойлера)
 * @param ledAlarm Ссылка-указатель на светодиод, индицирующий режим работы охраны
 * @param ledRXIR Ссылка-указатель на светодиод, индицирующий получение события с приемника RXIR. 
